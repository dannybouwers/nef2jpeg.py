...
def solveLinearEquation(IN, wx, wy, lamda):
    ...
    Ax = scipy.sparse.spdiags(np.concatenate(
        (dxd1[:, None], dxd2[:, None]), axis=1).T, np.array([-k+r, -r]), k, k, format=matrixFormat)
    Ay = scipy.sparse.spdiags(np.concatenate(
        (dyd1[None, :], dyd2[None, :]), axis=0), np.array([-r+1, -1]), k, k, format=matrixFormat)
    D = 1 - (dx + dy + dxa + dya)
    A = ((Ax+Ay) + (Ax+Ay).conj().T + scipy.sparse.spdiags(D, 0, k, k, format=matrixFormat)).T
...
def nice(img, mu=0.5, a=-0.3293, b=1.1258):
    lamda = 0.5
    sigma = 5
    I = cv2.normalize(img.astype('float64'), None, 0.0, 1.0, cv2.NORM_MINMAX)
    ...
    t_b = np.max(I, axis=2)
    width, height = t_b.shape[:2]
    t_our = cv2.resize(tsmooth(cv2.resize(t_b, (height//2, width//2), interpolation=cv2.INTER_CUBIC), lamda, sigma),
